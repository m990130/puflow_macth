...
class TrainerModule(pl.LightningModule):
    ...
    def validation_epoch_end(self, batch):
        ...
        # Print time for every epoch
        time_elapse = self.timer.update()
        log_dict['Time'] = time_elapse % (60 * 60) / 60  # minute
        ...
        print_progress_log(self.epoch, { 'CD': log_dict['CD'], 'Time': log_dict['Time'] }, extra=[])
        self.epoch += 1
...
def model_specific_args():
    ...
    parser.add_argument('--normal_support', action='store_true', help='whether to support normal estimation')
...
def train(phase='Train', checkpoint_path: str=None, begin_checkpoint: str=None):
    ...
    comment = 'Baseline-pu1k'

    pu1k_dataset_cfg = OmegaConf.create({
        "data_path": 'data/pu1k_poisson_256_poisson_1024_pc_2500_patch50_addpugan.h5',
        'batch_size': 32,
        "num_point_patch": 256,
        'up_ratio': 4,
    ...
    datamodule = PU1kDataModule(pu1k_dataset_cfg)
    ...
    trainer_config = {
        ...
        'callbacks'            : [TimeTrainingCallback(), LightningProgressBar(), ShotdownDatasetCallback()],
    }
...
if __name__ == "__main__":
    ...
    checkpoint_path = 'runs/ckpt/cnf-puflow-x4-pu1k.ckpt'
